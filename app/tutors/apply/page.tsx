...
export default function TutorApplyPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const degreeCertificate = getFile(
        formData,
        "degree_certificate"
      );
      const highSchoolCertificate = getFile(
        formData,
      ...
      const cvUrl = await uploadDirect(
        cvFile,
        "cv"
      );
      ...
      const payload = {
        ...
        referee_1_name: String(
          formData.get("referee_1_name") || ""
        ).trim(),

        referee_1_email: String(
          formData.get("referee_1_email") || ""
        ).trim(),

        referee_1_phone: String(
          formData.get("referee_1_phone") || ""
        ).trim(),

        referee_2_name: String(
          formData.get("referee_2_name") || ""
        ).trim(),

        referee_2_email: String(
          formData.get("referee_2_email") || ""
        ).trim(),

        referee_2_phone: String(
          formData.get("referee_2_phone") || ""
        ).trim(),
    ...
    <main className="min-h-screen bg-white text-slate-900">
      <section className="mx-auto max-w-5xl px-6 py-16 lg:px-8 lg:py-20">
        ...
        <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">
          Apply to join The Alkebula School.
          Only approved tutors will be listed publicly
          and allowed to publish bookable availability.
        </p>
        ...
        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-slate-700">
          ...
          <p className="mt-1">
            Profile photo should be below{" "}
            <strong>5MB</strong>. CV and
            certificates should each be below{" "}
            <strong>10MB</strong>.
          </p>
        </div>
        ...
        >
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              ...
              <p className="mb-2 text-xs text-slate-500">
                JPG or PNG only. Maximum file size:
                5MB.
              </p>
          ...
          <div className="space-y-3 rounded-2xl border border-slate-200 p-5">
            <label className="flex items-center gap-3 text-sm">
              <input
                name="declaration_no_criminal_past"
                type="checkbox"
              />

              I declare that I have no criminal
              past.
            </label>

            <label className="flex items-center gap-3 text-sm">
              <input
                name="declaration_internet_15mbps"
                type="checkbox"
              />

              I declare that I have at least 15
              mbps internet connection.
            </label>

            <label className="flex items-center gap-3 text-sm">
              <input
                name="declaration_has_i5_laptop"
                type="checkbox"
              />

              I declare that I have at least an i5
              laptop.
            </label>

            <label className="flex items-center gap-3 text-sm">
              <input
                name="declaration_information_true"
                type="checkbox"
              />

              I commit that all submitted
