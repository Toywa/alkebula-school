...
export default function EducatorDetailPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (tutorError || !tutor) {
          setErrorMessage("Educator not found or not approved.");
          setLoading(false);
          return;
        }
        ...
        setErrorMessage("Failed to load educator profile.");
        setLoading(false);
      }
    }
  ...
  async function handleBookSlot(
    ...
    setBookingState((prev) => ({ ...prev, [slot.id]: "booking" }));
    ...
    try {
      ...
      setBookingState((prev) => ({
        ...prev,
        [slot.id]: `success|Booking created successfully.${amountText}`,
      }));
      ...
      setBookingState((prev) => ({
        ...prev,
        [slot.id]: error instanceof Error ? error.message : "Booking failed",
      }));
    }
  }
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-white px-6 py-20 text-slate-900">
        <div className="mx-auto max-w-5xl">Loading educator profile...</div>
      </main>
    );
  }
      ...
      <main className="min-h-screen bg-white px-6 py-20 text-slate-900">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mt-4 text-slate-600">
            {errorMessage || "This educator is not available."}
          </p>
          <Link
            href="/educators"
          ...
          <div className="mt-8 grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <div className="mt-8">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {(educator.subjects ?? []).map((subject: string) => (
                    <span key={subject} className="rounded-full bg-amber-50 px-3 py-1 text-sm text-amber-800">
                      {subject}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-8">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {(educator.curricula ?? []).map((item: string) => (
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-2xl font-semibold">Available May Slots</h2>
              <p className="mt-2 text-sm text-slate-600">
                Select an available slot and complete the booking form.
              </p>

              {slots.length === 0 ? (
                <p className="mt-6 rounded-xl bg-slate-50 p-4 text-slate-600">
                  No available slots currently listed for this educator.
                </p>
              ) : (
              ...
              <p className="mt-6 text-sm text-slate-500">
                Booking creates an invoice and sends confirmation emails automatically.
              </p>
            </div>
          </div>
        </div>
